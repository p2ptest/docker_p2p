...
struct TTaskReport
{
	...
	{
		int nRet = 0;
		int nVecNum = m_vecTaskState.size();
		nRet += bufMgr.PutInt(nVecNum);
		for(int i = 0; i < nVecNum; i ++)
		{
			nRet += bufMgr.PutString(m_vecTaskState[i].m_strGcid);
			nRet += bufMgr.PutChar(m_vecTaskState[i].m_cState);
		}
		return nRet;
	}
	int GetSize()
	{
		...
		for(int i = 0; i < nVecNum; i ++)
		{
			nLen += m_vecTaskState[i].m_strGcid.length() + 4;
			nLen += sizeof(m_vecTaskState[i].m_cState);
		}
		return nLen;
	}
};
...
int main(int argc, char *argv[])
{
	if (argc < 4)
	{
		printf("Usage:./%s host port tasknum taskgcid1 taskstate1 ...\n", argv[0]);
		return -1;
	}
	
	TCmdHead sendHead;
	TCmdHead recvHead;
	TTaskReport sendBody;
	TTaskReportRes recvBody;

	TStateInfo tTaskState;
	for(int i = 0; i < atol(argv[3]); i ++)
	{
		...
		tTaskState.m_strGcid = CToolKit::HexToBi(argv[4 + 2 * i], 40);
		tTaskState.m_cState= atoi(argv[5 + 2 * i]);
		sendBody.m_vecTaskState.push_back(tTaskState);
	}
