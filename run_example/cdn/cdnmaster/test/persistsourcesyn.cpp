...
struct TPaidSourceSynRes
{
	char m_cResult;
	vector<string> m_vecGcids;
	...
	{
		printf("m_cResult = %d\n", m_cResult);
		int nVecNum = (int)m_vecGcids.size();
		printf("source num = %d\n", nVecNum);
		for(int i = 0; i < nVecNum; i ++)
		{
			printf("task %d: gcid = %s\n", i, CToolKit::BiToHex(m_vecGcids[i].c_str(), m_vecGcids[i].length()).c_str());
		}
	}
};
...
int main(int argc, char *argv[])
{
	...
	TCmdHead sendHead;
	TCmdHead recvHead;
	TPaidSourceSyn sendBody;
	TPaidSourceSynRes recvBody;
