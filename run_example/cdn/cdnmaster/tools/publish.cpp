...
int main(int argc, char *argv[])
{
	if (argc != 5)
	{
		printf("Usage:%s host port Gcid Ifhot\n", argv[0]);
		return -1;
	}
	...
	string strHost = argv[1];
	string strIp;
	if(CLocalDomainParser::GetInstance()->ParseDomain(strHost.c_str(), strIp) != 0)
	{
		printf("parse domain %s fail\n", strHost.c_str());
		return -1;
	}
	nRet = tmpSocket.Connect(strIp, atoi(argv[2]));
	if (nRet != 0)
	{
	...
	nRet = recvHead.Decode(recvBuffer);
	nRet += recvBody.Decode(recvBuffer);
	if (nRet != 0 || recvBody.m_cResult != 0)
	{
		return -1;
	}
//	recvHead.Print();
