...
public:
	...
	virtual int ProcessOptionsMethod(const char* pPacket, int nPachetLen);
...
template<typename T>
int CHttpRecvAndSendTask<T>::ProcessOptionsMethod(const char* pPacket, int nPacketLen)
{
	int nRet = 0;
	nRet = m_DataProcess.ProcessOptionsReq(pPacket, nPacketLen);
	if (nRet != 0)
		return -1;
	return 0;
}
...
int CHttpRecvAndSendTask<T>::ParsePackets(char *pPacket, unsigned int uiPacketLen)
{
	...
	while (1)
	{
		...
		else if (0 == strMethod.compare("OPTIONS"))
		{
			m_nContentLen = 0;
			m_iBodyHaseRecvLen = 0;
			m_state = ReadHttpHead;
			nRet = ProcessOptionsMethod(m_sHttpHead.c_str(), (int)m_sHttpHead.length());
			if (nRet != 0)
			{
				WriteRunInfo::WriteLog("ProcessOptionsMethod Failed");
				WriteRunInfo::WriteLog("Buf = %s", m_sHttpHead.c_str());
				return -1;
			}
...
int CHttpRecvAndSendTask<T>::GetHttpHead()
{
	...
	do 
	{
		...
		if (0 == strMethod.compare("GET") || 0 == strMethod.compare("HEAD") || 0 == strMethod.compare("OPTIONS"))
		{
			m_nContentLen = 0;
			m_iBodyHaseRecvLen = 0;
		}
