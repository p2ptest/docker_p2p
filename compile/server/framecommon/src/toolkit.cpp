...
int CToolKit::ParseDomain(const string &sDomain, string &sIpAddr)
{
		...
		while (NULL != *pTmp)
		{
			...
			inet_ntop(AF_INET, &tmpAddr, strIpBuf, sizeof(strIpBuf));
	        vIp.push_back(strIpBuf); 
			++pTmp;
		}
...
int CToolKit::GetStringByLabel(const string &sSrc,const string &sLabel1,const string &sLabel2, string &sOut, unsigned int nBeginPos /* = 0 */, int nIfRetPos /* = 0 */, bool ifcasesens /* = true */)
{
        ...
        {  
            pTmp0= (ifcasesens)? strstr(pTmp, sLabel1.c_str()) : strcasestr(pTmp, sLabel1.c_str()) ;
            if (pTmp0 == NULL)
            {      
                return -1;
            }
        ...
        if (sLabel2 != "")
        {   
            pTmp1 = (ifcasesens)? strstr(pTmp0+sLabel1.length(), sLabel2.c_str()) : strcasestr(pTmp0+sLabel1.length(), sLabel2.c_str());
            if (pTmp1 == NULL)
            {
                return -1;
            }
